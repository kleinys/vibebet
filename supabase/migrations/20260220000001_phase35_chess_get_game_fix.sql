-- =============================================================================
-- Phase 35 COMPLETE bootstrap — run this if phase 35 failed partway.
-- Fixes: "cannot change return type" AND "column g.move_count does not exist"
...
-- 2) Helpers + game RPCs (from phase 35)
create or replace function public._abort_skill_duel(
  p_escrow_code text, p_creator_id uuid, p_opponent_id uuid, p_stake bigint,
  p_tx_kind text, p_tx_ref text, p_metadata jsonb default '{}'::jsonb
) returns void language plpgsql security definer set search_path = '' as $$
declare v_escrow uuid; v_tx_id uuid; v_pool bigint;
begin
  ...
  values (p_tx_kind || '_abort', p_tx_ref || ':abort', p_metadata, p_creator_id) returning id into v_tx_id;
...
begin
  ...
  if p_status = 'active' then
    update public.chess_games set fen = p_fen, current_turn_id = p_next_turn_id, move_count = v_new_count,
  ...
  if p_status = 'draw' then
    perform public._settle_skill_duel(public._chess_escrow_code(p_game_id), null, v_game.creator_id, v_game.opponent_id,
      v_game.stake, v_game.is_friendly, 'chess', true, 'chess_settle', 'chess:' || p_game_id::text,
      jsonb_build_object('chess_id', p_game_id));
    update public.chess_games set fen = p_fen, status = 'draw', winner_id = null, current_turn_id = null,
      result_reason = p_result, settled_at = now(), draw_offered_by = null where id = p_game_id;
    return;
  end if;
  perform public._settle_skill_duel(public._chess_escrow_code(p_game_id), p_winner_id, v_game.creator_id, v_game.opponent_id,
    v_game.stake, v_game.is_friendly, 'chess', false, 'chess_settle', 'chess:' || p_game_id::text,
    jsonb_build_object('chess_id', p_game_id));
  update public.chess_games set fen = p_fen, status = 'settled', winner_id = p_winner_id, current_turn_id = null,
    result_reason = p_result, settled_at = now(), draw_offered_by = null where id = p_game_id;
end; $$;
...
begin
  ...
  perform public._abort_skill_duel(public._chess_escrow_code(p_game_id), v_game.creator_id, v_game.opponent_id,
    v_game.stake, 'chess', 'chess:' || p_game_id::text, jsonb_build_object('chess_id', p_game_id));
...
begin
  ...
  update public.chess_games set status = 'draw', winner_id = null, current_turn_id = null,
    result_reason = 'agreed_draw', settled_at = now(), draw_offered_by = null where id = p_game_id;
end; $$;
...
returns table (
  ...
  order by coalesce(g.started_at, g.created_at) desc limit p_limit;
$$;
