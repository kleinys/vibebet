-- =============================================================================
-- Phase 24: Quick exit — flat 85% of cost basis back (15% fee), non-fast markets
-- =============================================================================
...
declare
  ...
  v_tx_id        uuid;
  v_trade_id     uuid;
  v_payout_bps   int := 8500; -- 85% back, 15% fee
begin
  ...
  if v_market.fast_asset is not null then
    raise exception 'quick exit not available on fast/auto markets — use sell or wait for resolution';
  end if;
  ...
  if v_cost_basis <= 0 then raise exception 'no cost basis to exit'; end if;
  ...
  if v_pool_balance < v_proceeds then
    raise exception 'market pool cannot cover quick exit right now';
  end if;

  insert into public.ledger_transactions (kind, external_ref, metadata, created_by)
  values (
    'quick_exit',
    'quick_exit:' || gen_random_uuid()::text,
    jsonb_build_object(
...
revoke execute on function public.quick_exit_shares(uuid, public.trade_side, bigint) from public;
grant  execute on function public.quick_exit_shares(uuid, public.trade_side, bigint) to authenticated;

insert into public.feature_flags (key, enabled, description)
values (
  'quick_exit_enabled',
  false,
  'Quick exit: get 85% of cost basis back on open markets (15% fee). Not on fast markets.'
)
