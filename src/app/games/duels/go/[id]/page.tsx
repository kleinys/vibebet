...
export default async function GoGamePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const gameUrl = watchSkillGameUrl("go", id);
      ...
      {canJoin ? (
        <div className="mt-8 rounded-xl border border-slate-500/20 bg-slate-500/5 p-5">
          <SkillGameAcceptButton
            gameKey="go"
            gameId={id}
            className="rounded-md bg-slate-600 px-4 py-2 text-sm text-white hover:bg-slate-500 disabled:opacity-50"
          />
        </div>
      ) : isCreatorWaiting ? (
        ...
        <div className="mt-8">
          ...
          {userWon && (
            <WinSharePanel
              ...
              headline="Won a Go duel on Vibebet"
            />
          )}
        </div>
      )}
